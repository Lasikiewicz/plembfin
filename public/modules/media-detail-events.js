import { HIDE_EPISODE_SPOILERS_KEY, state } from "./state.js?v=1.1.1.8.0";
import { escapeAttribute, formatDate, showTitleFrom, showName, slug, movieHref, movieTmdbHref, tvShowBaseHrefFromEpisode, tvShowTmdbHref, tvShowTvdbHref, normalizePlatformSource } from "./utils.js?v=1.1.1.8.0";
import { isCachedStorageImageUrl, proxiedArtworkUrl, rememberPosterLookup } from "./images.js?v=1.1.1.8.0";
import {
  openEditDateDialog,
...
} from "./edit-dialogs.js?v=1.1.1.8.0";
import {
  openWatchDatePrompt,
...
} from "./watch-action.js?v=1.1.1.8.0";
import { triggerRetrySync, loadSyncJobs, loadSyncHistory, showAvailIssuePopup, isWatchedHistoryAction } from "./sync.js?v=1.1.1.8.0";
import { renderExplorer, renderHistoryView, resolvedTmdbCache, refreshMovieExplorerInPlace, refreshHistoryViewInPlace } from "./explorer.js?v=1.1.1.8.0";
import {
  movieBySlugOrId,
...
import { fetchWatchedMovieByTmdb, syncRewatchHistoryToggle } from "./media-detail-movie.js?v=1.1.1.8.0";
import { addToWatchlist, removeFromWatchlist, openAddToListDialog, personalItemFromDetailDataset, refreshRenderedPersonalMediaControls, loadPersonalMedia } from "./personal-media.js?v=1.1.1.8.0";
import { addShowToUpNext, removeShowFromUpNext, upNextAttentionOptions } from "./up-next.js?v=1.1.1.8.0";
...
async function refreshAfterWatchDateDeletion() {
  ...
  document.addEventListener("click", async (event) => {
    ...
    if (editDateBtn) {
      ...
      openEditDateDialog(container, editDateBtn.dataset.editId, editDateBtn.dataset.watchedAt, async ({ watched_at = "", deleted = false } = {}) => {
        if (deleted) {
          await refreshAfterWatchDateDeletion();
          return;
        }
    ...
    if (seasonAccordion) {
      ...
      // The app shell keeps window/document fixed and scrolls .page-shell
      // internally instead - see scrollSeasonAccordionIntoView's doc comment.
      ...
      if (ctx?.show) {
        ...
        requestAnimationFrame(() => {
          ...
          if (nextSeason != null) {
            scrollSeasonAccordionIntoView(nextSeason);
