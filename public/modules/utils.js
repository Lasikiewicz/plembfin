...
export function platformIconUrl(value) {
  const normalized = normalizePlatformSource(value);
  if (normalized === "plembfin") return "/icons/plembfin.png?v=1.1.1.3.3";
  const extension = "svg";
  return `/icons/${normalized}.${extension}?v=1.1.1.3.3`;
}
...
export function platformIconMarkup(value, className = "source-badge-icon", wrapperClass = "source-badge-icon-set") {
  ...
  return `<span class="${escapeAttribute(wrapperClass)} theme-aware-icon-set" aria-hidden="true">
    <img class="${safeClassName} theme-aware-icon--light" src="/icons/plembfin-light.png?v=1.1.1.3.3" alt="" loading="eager" decoding="async" />
    <img class="${safeClassName} theme-aware-icon--dark" src="/icons/plembfin.png?v=1.1.1.3.3" alt="" loading="eager" decoding="async" />
  </span>`;
}
