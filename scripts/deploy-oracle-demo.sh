...
IMAGE="${1:?Usage: deploy-oracle-demo.sh IMAGE CONTAINER_NAME DATA_DIR HOST_PORT RUNTIME [TRAKS_SCRIPT_URL] [TRAKS_SITE_KEY] [TRAKS_REQUIRE_CONSENT] [GA_MEASUREMENT_ID] [GA_REQUIRE_CONSENT] [TRAKS_COLLECTOR_ORIGIN]}"
CONTAINER_NAME="${2:?Usage: deploy-oracle-demo.sh IMAGE CONTAINER_NAME DATA_DIR HOST_PORT RUNTIME [TRAKS_SCRIPT_URL] [TRAKS_SITE_KEY] [TRAKS_REQUIRE_CONSENT] [GA_MEASUREMENT_ID] [GA_REQUIRE_CONSENT] [TRAKS_COLLECTOR_ORIGIN]}"
DATA_DIR="${3:?Usage: deploy-oracle-demo.sh IMAGE CONTAINER_NAME DATA_DIR HOST_PORT RUNTIME [TRAKS_SCRIPT_URL] [TRAKS_SITE_KEY] [TRAKS_REQUIRE_CONSENT] [GA_MEASUREMENT_ID] [GA_REQUIRE_CONSENT] [TRAKS_COLLECTOR_ORIGIN]}"
HOST_PORT="${4:?Usage: deploy-oracle-demo.sh IMAGE CONTAINER_NAME DATA_DIR HOST_PORT RUNTIME [TRAKS_SCRIPT_URL] [TRAKS_SITE_KEY] [TRAKS_REQUIRE_CONSENT] [GA_MEASUREMENT_ID] [GA_REQUIRE_CONSENT] [TRAKS_COLLECTOR_ORIGIN]}"
RUNTIME="${5:?Usage: deploy-oracle-demo.sh IMAGE CONTAINER_NAME DATA_DIR HOST_PORT RUNTIME [TRAKS_SCRIPT_URL] [TRAKS_SITE_KEY] [TRAKS_REQUIRE_CONSENT] [GA_MEASUREMENT_ID] [GA_REQUIRE_CONSENT] [TRAKS_COLLECTOR_ORIGIN]}"
TRAKS_SCRIPT_URL="${6:-}"
TRAKS_SITE_KEY="${7:-}"
TRAKS_REQUIRE_CONSENT="${8:-true}"
GA_MEASUREMENT_ID="${9:-}"
GA_REQUIRE_CONSENT="${10:-true}"
TRAKS_COLLECTOR_ORIGIN="${11:-}"
...
run_runtime run "${runtime_arch_args[@]}" --detach \
  ...
  --env PLEMBFIN_TRAKS_COLLECTOR_ORIGIN="$TRAKS_COLLECTOR_ORIGIN" \
  --env PLEMBFIN_TRAKS_SITE_KEY="$TRAKS_SITE_KEY" \
  --env PLEMBFIN_TRAKS_REQUIRE_CONSENT="$TRAKS_REQUIRE_CONSENT" \
  --env PLEMBFIN_GA_MEASUREMENT_ID="$GA_MEASUREMENT_ID" \
  --env PLEMBFIN_GA_REQUIRE_CONSENT="$GA_REQUIRE_CONSENT" \
  --env TRUST_PROXY=1 \
