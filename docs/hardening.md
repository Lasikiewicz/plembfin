...
- If the token is ever exposed, rotate it in **Settings → Integrations → Rotate Webhook Token**. Rotations take effect immediately; update all media servers afterwards.
...
|--------|---------------|
| Admin password | **Settings → General → Change Login** |
| Webhook token | **Settings → Integrations → Rotate Webhook Token** |
