...
- `PLEMBFIN_TRAKS_SCRIPT_URL` / `PLEMBFIN_TRAKS_SITE_KEY` / `PLEMBFIN_TRAKS_REQUIRE_CONSENT` - optional public-demo Traks configuration; the tracker URL and site key are public, HTTPS is required, and consent defaults to required
- `PLEMBFIN_GA_MEASUREMENT_ID` / `PLEMBFIN_GA_REQUIRE_CONSENT` - optional public-demo GA4 configuration; only a valid `G-...` Measurement ID is exposed, and consent defaults to required
