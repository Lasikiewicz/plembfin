...
| File | What it is |
| --- | --- |
| `workflows/update-changelog.yml` (workflow name "Publish Main Release") | On every push to `main`: runs the full build gate, reads the version already committed by "Force to main" (`scripts/promote-alpha-to-main.js`, run locally before the push), builds/pushes the Docker image to GHCR tagged `latest` + the version, and creates or updates the matching formatted GitHub Release through `scripts/publish-github-release.js`. Does not write anything back to the branch. |
...
| `generate-release-notes.js` | Renders the committed main or alpha changelog entry into the formatted GitHub Release body: categorized changes, safe-start guidance, limitations, Docker tags, Windows installer status, project links, and tagged screenshots. Supports a local Markdown preview with `node scripts/generate-release-notes.js <main|alpha>`. |
