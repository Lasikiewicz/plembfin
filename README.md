...
| **Live Trakt sync** | Mark an item watched or unwatched in Trakt, Plex, Emby, Jellyfin, or Plembfin and distribute that state to every connected service; every individual Trakt rewatch is imported as its own history entry, episode writes resolve series IDs automatically, and Sync Now repairs drift against the complete Trakt snapshot. A large batch of episodes disappearing from one show at once is held back and re-checked on the next poll rather than trusted immediately, so a rate-limited or incomplete response from Trakt can't cascade a false unwatch across every connected platform |
